...
library(readr)
library(SnowballC)
twitter_data <- read_csv("D:/Coursera-social media/case study 2_text mining/results_madrid.csv")
...
text_clean = gsub("(RT|via)((?:\\b\\W*@\\w+)+)", "", text)   #remove retweet entitiles
text_clean = gsub("@\\w+", "", text_clean)                  # remove @people  
text_clean = gsub("[[:punct:]]", "", text_clean)            # remove puctuation symboles
text_clean = gsub("[[:digit:]]", "", text_clean)            #remove numbers
text_clean = gsub("http\\w+", "", text_clean)               # reomove links


#corpus = set of words in dataset
...
# document-MAtrix --> matrix that describe the frequency of terms that occur in a collection of documents(here it is tweets)
